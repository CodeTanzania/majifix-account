...
AccountSchema.statics.afterGet = function afterGet(mquery, results, done) {
  ...
  if (shouldFetch) {
    ...
    Account
      .fetchAndUpsert(identity, fetchedAt, function (error, account) {
        if (!error && account) {
...
AccountSchema.statics.verify = function verify(requestor, done) {
  ...
  accessor.account = _.toUpper(accessor.account);
  ...
  if (_.isEmpty(accessor.account) && _.isEmpty(accessor.identity)) {
  ...
  async.waterfall([
    ...
    function findAccountByIdentities(next) {
      const { account, identity } = accessor;
      const criteria = ({ $or: [{ number: account }, { identity: identity }] });
      Account
        .findOne(criteria, function (error, account) {
          if (!account) {
 ...
 * @name upsert
 * @function upsert
 * @description create or update existing account
 * @param {object} account valid account details
 * @param {function} done a callback to invoke on success or error
 * @return {Account|Error}
 * @since 0.1.0
 * @version 1.0.0
 * @static
 */
AccountSchema.statics.upsert = function upsert(account, done) {

  //ensure account
  let _account =
    _.merge({}, _.isFunction(account.toObject) ? account.toObject() :
      account);
  _account = _.omitBy(_account, isEmpty); //remove empty fields
  _account.number = _.toUpper(account.number); //ensure uppercased number
  _account.identity = _.toUpper(account.identity); //ensure uppercased identity
  ...
  async.waterfall([
    ...
    function findExistingByIdentities(next) {
      const { number, identity } = _account;
      const criteria = ({ $or: [{ number: number }, { identity: identity }] });
      Account.findOne(criteria, next);
    },

    function createOrUpdate(found, next) {

      //create account if not exists
      if (!found) {
        Account.post(_account, next);
      }
      ...
      //update if existing
        ...
        //upsert accessors
        _.forEach(_account.accessors, function (accessor) {
          if (!_.isEmpty(accessor.phone)) {
            found.upsertAccessor(accessor.phone, accessor);
          }
        });

        //unset accessors
        delete _account.accessors;
        delete _account.number;

        //TODO bills upsert

        //set other & patch
        found.put(_account, next);
...
AccountSchema.statics.fetch = function fetch(identity, fetchedAt, done) {
  ...
  if (hasArguments && canFetch) {
    Account
      .fetchAccount(identity, fetchedAt, function afteFetch(error,
        fetched) {
 ...
 * @static
 */
AccountSchema.statics.fetchAndUpsert =
  function fetchAndUpsert(identity, fetchedAt, done) {
    ...
    async.waterfall([
      ...
      //1.
      function fetchAccount(next) {
        Account.fetch(identity, fetchedAt, next);
      },

      //2.
      function upsertFetchedAccount(account, next) {
        let _account = ({ number: identity, identity: identity });
        _account = _.merge({}, _account, account);
        Account.upsert(_account, next);
      }
