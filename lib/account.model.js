...
AccountSchema.statics.afterGet = function afterGet(mquery, results, done) {
  ...
  const identity = (filter.number || filter.identity);
  ...
  if (shouldFetch) {
    ...
    async.waterfall([
      ...
      function fetchAccountFromProvider(next) {
        this.fetch(identity, fetchedAt, next);
      }.bind(this),

      function upsertProvidedAccount(account, next) {
        //upsert if found
        if (!_.isEmpty(account)) {
          this.upsert(account, next);
        }
        //continue
        else {
          next(null, null);
...
AccountSchema.statics.verify = function verify(requestor, done) {
  ...
  async.waterfall([
    ...
    function findAccountByNumber(next) {
      const criteria =
        ({ $or: [{ number: accessor.account }, { identity: accessor.identity }] });
      Account
...
AccountSchema.statics.upsert = function upsert(account, done) {
  ...
  //ensure account
  let _account =
    _.merge({}, _.isFunction(account.toObject) ? account.toObject() : account);
  _account = _.omitBy(_account, function (v) { return !v; }); //remove empty fields
  ...
  async.waterfall([
    ...
    function findExistingByAccountNumber(next) {
      Account.findOne({ number: account.number }, next);
    },
...
AccountSchema.statics.fetch = function fetch(identity, fetchedAt, done) {
  ...
  if (hasArguments && canFetch) {
    Account
      .fetchAccount(identity, fetchedAt, function afteFetch(error, fetched) {
        let _fetched = fetched;
        if (!error && !_.isEmpty(fetched)) {
          _fetched = _.merge({}, { fetchedAt: new Date() }, fetched);
          _fetched = _.omitBy(_fetched, function (v) { return !v; });
        }
