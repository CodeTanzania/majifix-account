...
const { phone, env, schema, models } = require('@codetanzania/majifix-common');
...
AccountSchema.methods.upsertAccessor =
  function upsertAccessor(phone, updates) {
...
AccountSchema.statics.fetchAndUpsert = function fetchAndUpsert(identity, done) {
  ...
  async.waterfall([
    ...
    function fetchAccount(account, next) {
      ...
      let _fetchedAt = fetchedAt;
      if(account){
        //reset to latest account fetch date
        _fetchedAt = (account.fetchedAt || _fetchedAt);

        //obtain latest billed date
        const _bills = _.orderBy(account.bills, 'period.billedAt', 'desc');
        let billedAt = ((_.first(_bills) || {}).period || {}).billedAt;
        billedAt = billedAt ? billedAt : _fetchedAt;
        billedAt = moment(billedAt).add(1, 'days').toDate();
    ...
    function findAccountJurisdiction(account, fetched, next) {
      ...
      //fetch jurisdiction
        ...
        //fetch jurisdiction
        Jurisdiction.findOne(criteria, function (error, jurisdiction) {
          _fetched.jurisdiction = (jurisdiction || account.jurisdiction);
